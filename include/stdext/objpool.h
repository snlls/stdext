...
#define objpool_obj_index(poolptr, ptr)					\
	((((intptr_t)(ptr)) - ((intptr_t)(poolptr)->objects.data)) / sizeof((poolptr)->objects.data[0]))

#define objpool_get(poolptr, index)		\
	(&((poolptr)->objects.data + (index))->v)

#define objpool_remove(poolptr, index) do { \
		if(!objpool_slot_is_set(poolptr, index))		\
			break;						\
		vector_get(&(poolptr)->objects, (index))->set = 0;	\
		*(vector_add(&(poolptr)->free_slots)) = (index);	\
	} while(0)


#define objpool_remove_obj(poolptr, objptr) objpool_remove(poolptr, objpool_obj_index(poolptr, objptr))
